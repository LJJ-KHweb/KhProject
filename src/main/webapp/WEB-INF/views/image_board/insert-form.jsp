        ...
        <form action="" 
            ...
            <script>
            	function loadImg(e, num){
					...
					// num : 몇 번째 요소인지 확인후 미리보기 할라고 받음
					console.log(e.files);
					//files : 첨부한 파일의 정보를 가지고있음
					// files.length : 첨부하면 1 취소 0
					//파일이 첨부되었을 경우 e.files[0]에 파일의 정보를 파악할수 있음
					if(e.files.length === 1){
						//선택한 파일을 읽어서 이미지를 띄울 수 있는 URL을 생성
						// 필요한 객체 == FileReader
						const reader = new FileReader();
						
						// reader 메소드 중 파일을 읽어서 URL로 만드는 메소드
						reader.readAsDataURL(e.files[0]);
						
						// 파일을 읽는 순간 고유한 URL이 담김
						// URL을 미리보기 영역의 img태그의 src속성값으로 대입할 것
						
						// 파일 읽기가 완료되면 실행할 이벤트 핸들러 정의
